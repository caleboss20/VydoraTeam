...
const SettingRow = ({ icon, label }: { icon: string; label: string }) => (
  <TouchableOpacity style={styles.settingRow} activeOpacity={0.65}>
    <View style={styles.settingIconWrap}>
      <Ionicons name={icon as any} size={moderateScale(18)} color={C.textMuted} />
    </View>
    <Text style={styles.settingLabel}>{label}</Text>
    <Ionicons name="chevron-forward-outline" size={moderateScale(16)} color={C.textDim} />
  </TouchableOpacity>
);
// ─── Screen ───────────────────────────────────────────────────────────────────
      ...
      >
        ...
        <View style={styles.card}>
          {ACCOUNT_ITEMS.map((item, i) => (
            <React.Fragment key={item.label}>
              ...
              {i < ACCOUNT_ITEMS.length - 1 && <View style={styles.rowDivider} />}
            </React.Fragment>
          ))}
        </View>
        {/* Support section */}
        ...
        <View style={styles.card}>
          {SUPPORT_ITEMS.map((item, i) => (
            <React.Fragment key={item.label}>
              <SettingRow icon={item.icon} label={item.label} />
              {i < SUPPORT_ITEMS.length - 1 && <View style={styles.rowDivider} />}
            </React.Fragment>
          ))}
        </View>
